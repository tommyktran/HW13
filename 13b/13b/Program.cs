...
namespace _13b
{
    ...
    class Program
    {
        ...
        static void Spelling(string word)
        {
            var periodicTable = new PeriodicTable();
            var symbols = new List<string> { };
            foreach (var element in periodicTable.Elements)
            {
                symbols.Add(element.Symbol.ToLower());
            }
            var result = new List<string> { };
            Spelling(symbols, word, result);

        }
        static bool Spelling(List<string> symbols, string word, List<string> result)
        {
            if (word == "")
            {
            ...
            if (symbols.Contains(word.Substring(0, 1).ToLower())) {
                string temp = word.Substring(0, 1).ToUpper();
                result.Add(temp);
                if (Spelling(symbols, word.Substring(1), result)) return true;
                result.RemoveAt(result.Count-1);
            }
            if (word.Length >= 2)
            {
                if (symbols.Contains(word.Substring(0, 2).ToLower()))
                {
                    string temp = word.Substring(0, 2).ToLower();
                    temp = char.ToUpper(temp[0]) + temp.Substring(1);
                    result.Add(temp);
                    if (Spelling(symbols, word.Substring(2), result)) return true;
                    result.RemoveAt(result.Count - 1);
                }
            }
